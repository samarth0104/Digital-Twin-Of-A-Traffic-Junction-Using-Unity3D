...
public class RandomObjectSpawner : MonoBehaviour
{
    ...
    void Start()
    {
        spawnCountFilePath = Application.dataPath + "/spawn_count1.txt"; // Path to the text file
    ...
    IEnumerator SpawnObjectsRoutine()
    {
        while (true)
        {
            ...
            for (int i = 0; i < spawnCount; i++)
            {
                ...
                Vector3 randomSpawnPosition = new Vector3(-4.7f, 0.78f, -62.7f);
                Instantiate(myObjects[randomIndex], randomSpawnPosition, Quaternion.identity);
            }

            // Read the total time from the text file
